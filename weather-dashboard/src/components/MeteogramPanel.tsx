...
const ChartTooltip = ({ active, payload, label, unit }: {
  ...
  return (
    <div className="chart-tooltip px-2 py-1 text-[10px] font-mono shadow-xl">
...
function StatCard({ icon: Icon, label, value, numeric, suffix, sub, tone, color }: {
  ...
  return (
    <div className={`metric-card border rounded p-2 transition-all ${toneClass(tone, numeric)}`}>
...
export default function MeteogramPanel() {
  ...
  return (
    <div className="flex-1 flex flex-col p-2 gap-1.5 overflow-y-auto min-h-0 [scrollbar-width:thin] [scrollbar-color:#404040_transparent]">
      ...
      <div className="wd-map-card p-2.5 rounded-lg flex justify-between items-start gap-2 shrink-0">
      ...
      {!error && insights.length > 0 && (
        <div className="chart-panel shrink-0">
      ...
      {tab === 'now' && !error && <>
        ...
        {chartData.length > 0 && <ErrorBoundary fallbackLabel="Chart failed"><div className="chart-panel shrink-0"><div className="flex items-center justify-between mb-1"><div className="flex items-center gap-1 text-[9px] text-neutral-500 uppercase tracking-wider"><TrendingUp size={9} /> 24-hr Temperature</div><div className="flex items-center gap-2 text-[8px] text-neutral-600"><span>Peak {formatTemp(peakTemp.temp, tempUnit)} at {peakTemp.time}</span>{hy && <span className="flex items-center gap-1"><History size={8} /> vs last year</span>}</div></div><div className="h-28"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: -24 }}><CartesianGrid strokeDasharray="2 4" stroke="var(--chart-grid)" /><XAxis dataKey="time" tick={{ fontSize: 8, fill: 'var(--text-soft)' }} interval={3} tickLine={false} axisLine={false} /><YAxis tick={{ fontSize: 8, fill: 'var(--text-soft)' }} tickLine={false} axisLine={false} tickFormatter={(v) => tempUnit === 'C' ? `${v}°` : `${celsiusToF(v)}°`} /><Tooltip content={(p) => <ChartTooltip {...p} unit={tempUnit} />} cursor={{ stroke: 'var(--chart-grid)' }} /><Line type="monotone" dataKey="temp" name="This year" stroke="var(--chart-temp)" strokeWidth={2.4} dot={false} activeDot={{ r: 4 }} /><ReferenceDot x={peakTemp.time} y={peakTemp.temp} r={3} fill="var(--accent-3)" stroke="var(--text-strong)" />{hy && <Line type="monotone" dataKey="tempLY" name="Last year" stroke="#6b7280" strokeWidth={1} strokeDasharray="3 2" dot={false} activeDot={{ r: 2 }} />}</LineChart></ResponsiveContainer></div></div></ErrorBoundary>}

        {chartData.length > 0 && <ErrorBoundary fallbackLabel="Chart failed"><div className="chart-panel shrink-0"><div className="flex items-center justify-between mb-1"><div className="flex items-center gap-1 text-[9px] text-neutral-500 uppercase tracking-wider"><CloudRain size={9} /> 24-hr Precipitation + Probability</div><div className="text-[8px] text-neutral-600">Peak {peakRain.rain.toFixed(1)} mm at {peakRain.time}</div></div><div className="h-24"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: -24 }}><CartesianGrid strokeDasharray="2 4" stroke="var(--chart-grid)" vertical={false} /><XAxis dataKey="time" tick={{ fontSize: 8, fill: 'var(--text-soft)' }} interval={3} tickLine={false} axisLine={false} /><YAxis yAxisId="rain" tick={{ fontSize: 8, fill: 'var(--text-soft)' }} tickLine={false} axisLine={false} /><YAxis yAxisId="prob" orientation="right" domain={[0, 100]} tick={{ fontSize: 8, fill: 'var(--text-soft)' }} tickLine={false} axisLine={false} tickFormatter={(v) => `${v}%`} /><Tooltip content={(p) => <ChartTooltip {...p} unit={tempUnit} />} cursor={{ fill: 'var(--chart-fill)' }} /><Bar yAxisId="rain" dataKey="rain" name="Rain mm" fill="var(--chart-rain)" opacity={0.88} radius={[3, 3, 0, 0]} /><Line yAxisId="prob" type="monotone" dataKey="prob" name="Probability" stroke="var(--chart-prob)" strokeWidth={2} dot={false} activeDot={{ r: 3 }} /></BarChart></ResponsiveContainer></div></div></ErrorBoundary>}
      </>}
