...
export function buildOverlayLayers(
  ...
  if (activeLayers.includes('radar') && rainviewerTs) {
    push(
      ...
      { 'raster-opacity': 0.46, 'raster-contrast': -0.08, 'raster-saturation': -0.06, 'raster-brightness-min': 0.08, 'raster-brightness-max': 0.82, 'raster-resampling': 'linear' },
  ...
  if (activeLayers.includes('storms') && rainviewerTs) {
    push(
      ...
      { 'raster-opacity': 0.52, 'raster-contrast': -0.02, 'raster-saturation': 0.02, 'raster-brightness-min': 0.08, 'raster-brightness-max': 0.84, 'raster-resampling': 'linear' },
      { attribution: RAINVIEWER_ATTRIBUTION, minzoom: 0, maxzoom: RAINVIEWER_RADAR_MAX_ZOOM }
    );
  }
  ...
  if (activeLayers.includes('satellite') && satelliteTs) {
    push(
      ...
      { 'raster-opacity': 0.24, 'raster-contrast': -0.16, 'raster-saturation': -0.28, 'raster-brightness-min': 0.10, 'raster-brightness-max': 0.72, 'raster-resampling': 'linear' },
  ...
  if (activeLayers.includes('wind')) {
    push('wind-src', [`https://tile.openweathermap.org/map/wind_new/{z}/{x}/{y}.png?appid=${owmKey}`], 'wind-layer', { 'raster-opacity': 0.34, 'raster-hue-rotate': 200, 'raster-saturation': -0.04, 'raster-brightness-max': 0.82, 'raster-resampling': 'linear' }, owmSourceOptions);
  }

  if (activeLayers.includes('temp')) {
    push('temp-src', [`https://tile.openweathermap.org/map/temp_new/{z}/{x}/{y}.png?appid=${owmKey}`], 'temp-layer', { 'raster-opacity': 0.36, 'raster-contrast': -0.08, 'raster-brightness-max': 0.80, 'raster-resampling': 'linear' }, owmSourceOptions);
  }

  if (activeLayers.includes('precip')) {
    push('precip-src', [`https://tile.openweathermap.org/map/precipitation_new/{z}/{x}/{y}.png?appid=${owmKey}`], 'precip-layer', { 'raster-opacity': 0.38, 'raster-contrast': -0.10, 'raster-saturation': -0.08, 'raster-brightness-max': 0.78, 'raster-resampling': 'linear' }, owmSourceOptions);
  }

  if (activeLayers.includes('clouds')) {
    push('clouds-src', [`https://tile.openweathermap.org/map/clouds_new/{z}/{x}/{y}.png?appid=${owmKey}`], 'clouds-layer', { 'raster-opacity': 0.22, 'raster-contrast': -0.18, 'raster-saturation': -0.28, 'raster-brightness-max': 0.68, 'raster-resampling': 'linear' }, owmSourceOptions);
  }

  if (activeLayers.includes('pressure')) {
    push('pressure-src', [`https://tile.openweathermap.org/map/pressure_new/{z}/{x}/{y}.png?appid=${owmKey}`], 'pressure-layer', { 'raster-opacity': 0.32, 'raster-contrast': -0.08, 'raster-brightness-max': 0.82, 'raster-resampling': 'linear' }, owmSourceOptions);
  }
