    ...
    <div className="grid gap-4">
        ...
        {filters.map((item) => (
          <button
            ...
            className={`rounded-full border px-3 py-2 text-xs font-semibold ${
              filter === item ? "border-cyan-300 bg-cyan-300/10 text-cyan-100" : "border-slate-700 text-slate-400"
            }`}
          >
            {item}
          </button>
      ...
      <div className="grid gap-3">
        {filteredPlaces.map((place) => (
          <div key={place.id} className="rounded-2xl border border-slate-700/70 bg-slate-950/50 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="font-semibold text-white">{place.name}</p>
                <p className="mt-1 text-xs text-slate-400">{place.type} · {place.distanceKm.toFixed(1)} km · ★ {place.rating}</p>
              </div>
              <span className={`rounded-full px-2 py-1 text-[10px] font-bold ${place.openNow ? "bg-emerald-300 text-slate-950" : "bg-slate-700 text-slate-200"}`}>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {place.cardPromotions.map((promo) => (
                <span key={promo} className="rounded-full bg-violet-300/10 px-2 py-1 text-[10px] text-violet-100">
                  {promo}
                </span>
              ))}
            </div>
